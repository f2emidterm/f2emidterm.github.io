...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    function showUsedProducts() {
        ...
        listContainer.innerHTML = ""; // 清空清單
        
        // 1. 抓取目前畫布上所有的貼紙
        const allStickers = document.querySelectorAll(".placed-sticker");
        
        // 用來記錄已經加入過的商品 ID，避免重複顯示
        const addedIds = new Set();
        let hasItems = false;

        allStickers.forEach(stickerDiv => {
            const pid = stickerDiv.dataset.productId;
            const imgTag = stickerDiv.querySelector("img");
            
            // 只要有 ID 且這張圖還沒被加過
            if (pid && imgTag && !addedIds.has(pid)) {
                
                // 標記這個 ID 已處理
                addedIds.add(pid);
                hasItems = true;

                // 嘗試從資料庫抓名稱，抓不到就顯示預設文字
                const productName = (productsMap[pid] && productsMap[pid].name) ? productsMap[pid].name : "裝飾貼紙";

                // ★ 這裡直接用 stickerDiv 裡面的圖片網址 (imgTag.src)
                const itemHTML = `
                    <a href="product.html?id=${pid}" class="used-product-item" target="_blank">
                        <img src="${imgTag.src}" class="used-product-thumb" style="object-fit: contain;">
                        <div class="used-product-info">
                            <div class="used-product-name">${productName}</div>
                        </div>
                    </a>
                `;
        ...
        setTimeout(() => {
            const closeDropdown = (e) => {
                 if (!popup.contains(e.target) && !document.getElementById("downloadBtn").contains(e.target)) {
